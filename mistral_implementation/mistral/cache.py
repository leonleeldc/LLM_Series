...
def unrotate(cache: torch.Tensor,
             seqlen: int) -> torch.Tensor:  # seqlen is the total number of tokens cached so far, including the overwritten one. This is needed to calculate the rotation point of the cache
  ...
  if seqlen < cache.shape[
    0]:  # If the total sequence length so far is smaller than the cache size, then just return the first seqlen elements, as the cache didn't have any rotations yet
    return cache[:seqlen]
  elif position == 0:
    return cache
  else:
    return torch.cat([cache[position:], cache[:position]],
                     dim=0)  # Select the unrotated elements from the cache around the pivot point


class CacheView:
  def __init__(self, cache_k: torch.Tensor, cache_v: torch.Tensor, metadata: RotatingCacheInputMetadata,
  ...
  def update(self, xk: torch.Tensor, xv: torch.Tensor):
    ...
    flat_cache_k = self.cache_k.view(-1, n_kv_heads,
                                     head_dim)  # (Max_Batch_Size, Sliding_Window_Size, N_Heads_KV, Head_Dim) --> (Max_Batch_Size * Sliding_Window_Size, N_Heads_KV, Head_Dim)
    flat_cache_v = self.cache_v.view(-1, n_kv_heads,
                                     head_dim)  # (Max_Batch_Size, Sliding_Window_Size, N_Heads_KV, Head_Dim) --> (Max_Batch_Size * Sliding_Window_Size, N_Heads_KV, Head_Dim)
  ...
  def interleave_kv(self, xk: torch.Tensor, xv: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    xk = torch.split(xk,
                     self.metadata.seqlens)  # (Seq1+Seq2+Seq3, N_Heads_KV, Head_Dim) --> [(Seq1, N_Heads_KV, Head_Dim), (Seq2, N_Heads_KV, Head_Dim), (Seq3, N_Heads_KV, Head_Dim)]
    xv = torch.split(xv,
                     self.metadata.seqlens)  # (Seq1+Seq2+Seq3, N_Heads_KV, Head_Dim) --> [(Seq1, N_Heads_KV, Head_Dim), (Seq2, N_Heads_KV, Head_Dim), (Seq3, N_Heads_KV, Head_Dim)]
    ...
    cache_k = [unrotate(t, s) for t, s in
               zip(self.cache_k, self.kv_seqlens)]  # Currently cached elements, already unrotated, one for each prompt
    cache_v = [unrotate(t, s) for t, s in
               zip(self.cache_v, self.kv_seqlens)]  # Currently cached elements, already unrotated, one for each prompt

    interleaved_k = interleave_list(cache_k,
                                    xk)  # Appends the incoming keys and values to the currently cached elements (one for each prompt)
    interleaved_v = interleave_list(cache_v,
                                    xv)  # Appends the incoming keys and values to the currently cached elements (one for each prompt)
