from mistral_implementation.mistral.cache import RotatingBufferCache
import logging
import torch
import fire
from typing import List
from pathlib import Path
from mistral_implementation.mistral.mistral import Transformer
from mistral_implementation.mistral.tokenizer import Tokenizer
...
def generate(prompts: List[str], model: Transformer, tokenizer: Tokenizer, *, max_tokens: int, temperature: float,
  ...
  for s in range(0, max_prompt_len, chunk_size):
    ...
    prelogits = model.forward(
      ...
      # Concatenate all the tokens in the current chunk (of all the prompts) in a single tensor
